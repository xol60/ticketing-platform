 ...
 *
 * Routing logic:
 *  - Active read-only transaction → slave
 *  - Write transaction or no transaction → master
 *
 * Services annotate read-only methods with @Transactional(readOnly = true)
 * which sets the transaction read-only flag, routing to the slave automatically.
 */
@Configuration
